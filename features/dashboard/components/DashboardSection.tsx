const DashboardSection = () => {
  return (
    <div className="flex h-full flex-col gap-6 overflow-x-auto rounded-xl p-6">
        ...
        {Array.from({length: 3}).map((_, index) => (
          <div key={index} className="relative aspect-video overflow-hidden rounded-xl border border-sidebar-border/70 dark:border-sidebar-border">
            <svg className="absolute inset-0 size-full stroke-neutral-900/20 dark:stroke-neutral-100/20" fill="none">
              <defs>
                <pattern id="_r_c_" x="0" y="0" width="10" height="10" patternUnits="userSpaceOnUse">
                  <path d="M-3 13 15-5M-5 5l18-18M-1 21 17 3"></path>
                </pattern>
              </defs>
              <rect stroke="none" fill="url(#_r_c_)" width="100%" height="100%"></rect>
            </svg>
          </div>
        ))}
      </div>
